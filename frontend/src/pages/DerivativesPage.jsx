...
/* ─── Per-trade countdown timer ─────────────────────────────────────────── */
const TradeCountdown = ({ openedAt, duration }) => {
  const dur = duration || 60;
  const calc = () => Math.max(0, dur - Math.floor((Date.now() - new Date(openedAt).getTime()) / 1000));
  ...
  }, [openedAt, duration]); // eslint-disable-line react-hooks/exhaustive-deps

  if (remaining <= 0) {
    return <span className="text-[10px] text-slate-500 italic">Pending result…</span>;
  }
  ...
  return (
    <div className="flex flex-col gap-1 min-w-[72px]">
      <div className="flex items-center gap-1.5">
        <span className="text-[11px] font-mono font-bold" style={{ color }}>{remaining}s</span>
        <span className="text-[10px] text-slate-500">/ {dur}s</span>
      </div>
...
const DerivativesPage = () => {
  ...
  useEffect(() => {
    ...
    tradeService.getMyTrades()
      .then(({ trades: t }) => { if (!cancelled) setTrades(t.filter((x) => !x.type || x.type === 'derivatives')); })
      .catch(() => {});
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { trade, balance } = await tradeService.executeTrade({
        symbol, side, amount: a, entryPrice: selectedCoin?.price || null, duration, type: 'derivatives',
      });
      ...
      tradeService.getMyTrades()
        .then(({ trades: t }) => setTrades(t.filter((x) => !x.type || x.type === 'derivatives')))
        .catch(() => {});
    } finally {
      ...
      <div className="mx-auto max-w-7xl space-y-4">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between rounded-2xl bg-[#0D1421] ring-1 ring-white/[0.06] px-5 py-4">
          <div>
            ...
            <h1 className="mt-0.5 text-xl font-bold text-white">Derivatives</h1>
          </div>
          ...
          <div className="rounded-2xl bg-[#0D1421] ring-1 ring-white/[0.06] p-3 md:p-5 flex flex-col gap-3">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-3">
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? 'Opening...' : `${side === 'buy' ? '▲ Up' : '▼ Down'} · ${duration}s`}
              </button>
            </form>
                  ...
                  {trades.map((t) => (
                    <tr key={t._id} className="hover:bg-slate-800/20 transition-colors">
                      ...
                      <td className="py-2.5 pr-4">
                        {t.status === 'pending'
                          ? <TradeCountdown openedAt={t.openedAt || t.createdAt} duration={t.duration || 60} />
                          : <span className="text-slate-500">{t.duration ? `${t.duration}s` : '—'}</span>}
                      </td>
                      <td className="py-2.5 pr-4">
