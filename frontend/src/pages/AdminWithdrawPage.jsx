...
const AdminWithdrawPage = () => {
  ...
  const handleAction = async (id, action) => {
    ...
    setError('');
    try {
      if (action === 'approve') await requestService.approveWithdraw(id, notes[id]);
      else                      await requestService.rejectWithdraw(id, notes[id]);
      await load();
    } catch (err) {
  ...
  const statusStyle = (s) =>
    ...
    'bg-amber-500/20 text-amber-400 border-amber-500/20 animate-pending-glow';
          ...
          <div className="space-y-2">
            ...
            {done.map((r) => (
              <div key={r._id} className="rounded-2xl bg-[#0D1421] ring-1 ring-white/[0.04] px-4 py-3 flex items-center justify-between gap-4 flex-wrap">
                <div>
                  <p className="text-sm font-semibold text-white">{r.userId?.name} <span className="text-slate-500 font-normal">· ${r.amount.toFixed(2)}</span></p>
